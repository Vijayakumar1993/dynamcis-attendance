...
public class LoginController {
    ...
    public String updateLogin(@ModelAttribute Users user, @RequestParam("role") String role, Model model){
        boolean result = loginServices.createLogin(user, Roles.valueOf(role));
        if(!result){
    ...
    public String createLoginSetup(@RequestParam("customerId") String customerId, @RequestParam("role") String role, @RequestParam("username") String username, @RequestParam("password") String password, Model model){
        Customer customer = repostitary.findById(Long.parseLong(customerId)).get();
        Users users = new Users(username, password, true);
        users.setCustomerId(customer);
        boolean result = loginServices.createLogin(users, Roles.valueOf(role));
        if(!result){
            model.addAttribute("customer", customer);
