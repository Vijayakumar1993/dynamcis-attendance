    ...
    <div class="card shadow-sm p-4">
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-12">
                ...
                <input type="text" class="form-control" id="configName" name="configName" placeholder="Enter Config Name" <#if configuration?has_content>value="${configuration.configName?if_exists}"   <#else> value="${configName?if_exists}"  </#if>>
            </div>
        </div>

        <div class="mb-3">
            <label for="configKey" class="form-label">Config Key</label>
            <input type="text" class="form-control" id="configKey" name="configKey" placeholder="Enter Config Key" <#if configuration?has_content>value="${configuration.configKey?if_exists}" <#else>value="${configKey?if_exists}"  </#if>>
        </div>
